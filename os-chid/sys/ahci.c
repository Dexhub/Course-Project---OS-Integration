...
int write_interface(HBA_PORT *port, DWORD startl, DWORD starth, DWORD count, QWORD buf)
{
   ...
   // printf("\n Writing interface:34: addr : %p ",addr);
    HBA_CMD_HEADER *cmdheader = (HBA_CMD_HEADER*)(AHCI_KERN_BASE + addr);
    cmdheader += slot;
    cmdheader->cfl = sizeof(FIS_REG_H2D)/sizeof(DWORD);     // Command FIS size
    cmdheader->w = 1;               // Writing to device  -=> HK for write  w = 1
   ...
   // printf("\n Writing interface:45: addr : %p ",addr);
...
int read_interface(HBA_PORT *port, DWORD startl, DWORD starth, DWORD count, QWORD buf)
{
    ...
    //cmdheader = (HBA_CMD_HEADER*)(AHCI_KERN_BASE+buf);
    //printf(" \n HBA_CMD_HEADER : %p                   ", cmdheader);
